...
function getMonthlyRevenue(glData: GL[], year: number): number[] {
  ...
  glData.forEach((item: GL) => {
    ...
    if (item.Acc_No && item.Acc_No.startsWith('6') && item.Date) {
      const date = new Date(item.Date);
      if (date.getFullYear() === year) {
        const month = date.getMonth();
        monthly[month] += (parseFloat(item.Dibt) || 0) - (parseFloat(item.Cridt) || 0);
      }
    }
  });
  return monthly;
      ...
      datasets: [
        {
          ...
          fill: true,
          backgroundColor: 'rgba(0,200,140,0.2)', // green area
        ...
        },
        {
          ...
          fill: true,
          backgroundColor: 'rgba(255,99,132,0.2)', // red area
  ...
  const options: import('chart.js').ChartOptions<'line'> = {
    ...
    plugins: {
      ...
      title: {
        ...
        display: true,
        text: 'Year over Year Expenses Analysis',
